...
HB_FUNC( QCOREAPPLICATION_ONABOUTTOQUIT )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(aboutToQuit()), s, SLOT(aboutToQuit()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "aboutToQuit()";
    ret = object->disconnect(object, SIGNAL(aboutToQuit()), s, SLOT(aboutToQuit()) );
    if( ret )
    {
