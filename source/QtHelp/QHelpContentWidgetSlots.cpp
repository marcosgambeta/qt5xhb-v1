...
HB_FUNC( QHELPCONTENTWIDGET_ONLINKACTIVATED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "linkActivated(QUrl)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(linkActivated(QUrl)), s, SLOT(linkActivated(QUrl)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "linkActivated(QUrl)";
    ret = object->disconnect(object, SIGNAL(linkActivated(QUrl)), s, SLOT(linkActivated(QUrl)) );
    if( ret )
    {
