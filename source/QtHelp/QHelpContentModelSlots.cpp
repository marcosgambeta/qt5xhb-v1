...
HB_FUNC( QHELPCONTENTMODEL_ONCONTENTSCREATED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "contentsCreated()";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(contentsCreated()), s, SLOT(contentsCreated()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "contentsCreated()";
    ret = object->disconnect(object, SIGNAL(contentsCreated()), s, SLOT(contentsCreated()) );
    if( ret )
    {
...
HB_FUNC( QHELPCONTENTMODEL_ONCONTENTSCREATIONSTARTED )
{
  ...
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "contentsCreationStarted()";
    bool connected = Signals_is_signal_connected( object, signal );
    if( !connected )
    {
      PHB_ITEM codeblock = hb_itemNew( hb_param( 1, HB_IT_BLOCK | HB_IT_SYMBOL ) );
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(contentsCreationStarted()), s, SLOT(contentsCreationStarted()) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "contentsCreationStarted()";
    ret = object->disconnect(object, SIGNAL(contentsCreationStarted()), s, SLOT(contentsCreationStarted()) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
