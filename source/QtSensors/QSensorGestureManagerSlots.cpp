...
HB_FUNC( QSENSORGESTUREMANAGER_ONNEWSENSORGESTUREAVAILABLE )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "newSensorGestureAvailable()";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(newSensorGestureAvailable()), s, SLOT(newSensorGestureAvailable()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "newSensorGestureAvailable()";
    ret = object->disconnect(object, SIGNAL(newSensorGestureAvailable()), s, SLOT(newSensorGestureAvailable()) );
    if( ret )
    {
