...
HB_FUNC( QWIZARDPAGE_ONCOMPLETECHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "completeChanged()";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(completeChanged()), s, SLOT(completeChanged()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "completeChanged()";
    ret = object->disconnect(object, SIGNAL(completeChanged()), s, SLOT(completeChanged()) );
    if( ret )
    {
