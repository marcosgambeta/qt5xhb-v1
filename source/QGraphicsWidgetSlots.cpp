...
HB_FUNC( QGRAPHICSWIDGET_ONGEOMETRYCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "geometryChanged()";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(geometryChanged()), s, SLOT(geometryChanged()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "geometryChanged()";
    ret = object->disconnect(object, SIGNAL(geometryChanged()), s, SLOT(geometryChanged()) );
    if( ret )
    {
