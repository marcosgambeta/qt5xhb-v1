...
HB_FUNC( QABSTRACTAUDIOINPUT_ONERRORCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "errorChanged(QAudio::Error)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(errorChanged(QAudio::Error)), s, SLOT(errorChanged(QAudio::Error)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "errorChanged(QAudio::Error)";
    ret = object->disconnect(object, SIGNAL(errorChanged(QAudio::Error)), s, SLOT(errorChanged(QAudio::Error)) );
    if( ret )
    {
...
HB_FUNC( QABSTRACTAUDIOINPUT_ONSTATECHANGED )
{
  ...
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "stateChanged(QAudio::State)";
    bool connected = Signals_is_signal_connected( object, signal );
    if( !connected )
    {
      PHB_ITEM codeblock = hb_itemNew( hb_param( 1, HB_IT_BLOCK | HB_IT_SYMBOL ) );
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(stateChanged(QAudio::State)), s, SLOT(stateChanged(QAudio::State)) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "stateChanged(QAudio::State)";
    ret = object->disconnect(object, SIGNAL(stateChanged(QAudio::State)), s, SLOT(stateChanged(QAudio::State)) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
...
HB_FUNC( QABSTRACTAUDIOINPUT_ONNOTIFY )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(notify()), s, SLOT(notify()) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "notify()";
    ret = object->disconnect(object, SIGNAL(notify()), s, SLOT(notify()) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
