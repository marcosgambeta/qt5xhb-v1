...
HB_FUNC( QMEDIASTREAMSCONTROL_ONACTIVESTREAMSCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "activeStreamsChanged()";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(activeStreamsChanged()), s, SLOT(activeStreamsChanged()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "activeStreamsChanged()";
    ret = object->disconnect(object, SIGNAL(activeStreamsChanged()), s, SLOT(activeStreamsChanged()) );
    if( ret )
    {
...
HB_FUNC( QMEDIASTREAMSCONTROL_ONSTREAMSCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(streamsChanged()), s, SLOT(streamsChanged()) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "streamsChanged()";
    ret = object->disconnect(object, SIGNAL(streamsChanged()), s, SLOT(streamsChanged()) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
