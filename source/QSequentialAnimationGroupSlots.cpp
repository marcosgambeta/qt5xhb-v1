...
HB_FUNC( QSEQUENTIALANIMATIONGROUP_ONCURRENTANIMATIONCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "currentAnimationChanged(QAbstractAnimation*)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(currentAnimationChanged(QAbstractAnimation*)), s, SLOT(currentAnimationChanged(QAbstractAnimation*)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "currentAnimationChanged(QAbstractAnimation*)";
    ret = object->disconnect(object, SIGNAL(currentAnimationChanged(QAbstractAnimation*)), s, SLOT(currentAnimationChanged(QAbstractAnimation*)) );
    if( ret )
    {
