...
#define GET_PTR_FROM_SELF(p)                                                                                           \
  QBluetoothSocket *p = qobject_cast<QBluetoothSocket *>(Qt5xHb::getQObjectPointerFromSelfItem())
...
HB_FUNC_STATIC(QBLUETOOTHSOCKET_CONNECTTOSERVICE)
{
  if (ISBETWEEN(1, 2) && ISQBLUETOOTHSERVICEINFO(1) && ISNUMORNIL(2)) {
    ...
    if (obj != NULL) {
      obj->connectToService(*PQBLUETOOTHSERVICEINFO(1),
  ...
  if (obj != NULL) {
    ...
    if (ISBETWEEN(2, 4) && HB_ISNUM(1) && HB_ISNUM(2) && ISNUMORNIL(3) && ISNUMORNIL(4)) {
      ...
      RBOOL(obj->setSocketDescriptor(PINT(1), (QBluetoothServiceInfo::Protocol)hb_parni(2),
                                     HB_ISNIL(3) ? (QBluetoothSocket::SocketState)QBluetoothSocket::ConnectedState
                                                 : (QBluetoothSocket::SocketState)hb_parni(3),
...
#define CONNECT_SIGNAL(signal) QBluetoothSocketSlots_connect_signal(signal, signal)
...
{
#if (QT_VERSION >= QT_VERSION_CHECK(5, 2, 0))
  CONNECT_SIGNAL("error(QBluetoothSocket::SocketError)");
#else
  hb_retl(false);
#endif
}
...
{
#if (QT_VERSION >= QT_VERSION_CHECK(5, 2, 0))
  CONNECT_SIGNAL("stateChanged(QBluetoothSocket::SocketState)");
#else
  hb_retl(false);
#endif
}
