...
HB_FUNC( QSTANDARDITEMMODEL_ONITEMCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "itemChanged(QStandardItem*)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(itemChanged(QStandardItem*)), s, SLOT(itemChanged(QStandardItem*)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "itemChanged(QStandardItem*)";
    ret = object->disconnect(object, SIGNAL(itemChanged(QStandardItem*)), s, SLOT(itemChanged(QStandardItem*)) );
    if( ret )
    {
