...
HB_FUNC( QSCRIPTENGINEDEBUGGER_ONEVALUATIONRESUMED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "evaluationResumed()";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(evaluationResumed()), s, SLOT(evaluationResumed()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "evaluationResumed()";
    ret = object->disconnect(object, SIGNAL(evaluationResumed()), s, SLOT(evaluationResumed()) );
    if( ret )
    {
...
HB_FUNC( QSCRIPTENGINEDEBUGGER_ONEVALUATIONSUSPENDED )
{
  ...
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "evaluationSuspended()";
    bool connected = Signals_is_signal_connected( object, signal );
    if( !connected )
    {
      PHB_ITEM codeblock = hb_itemNew( hb_param( 1, HB_IT_BLOCK | HB_IT_SYMBOL ) );
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(evaluationSuspended()), s, SLOT(evaluationSuspended()) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "evaluationSuspended()";
    ret = object->disconnect(object, SIGNAL(evaluationSuspended()), s, SLOT(evaluationSuspended()) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
