...
HB_FUNC( QOPENGLCONTEXT_ONABOUTTOBEDESTROYED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "aboutToBeDestroyed()";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(aboutToBeDestroyed()), s, SLOT(aboutToBeDestroyed()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "aboutToBeDestroyed()";
    ret = object->disconnect(object, SIGNAL(aboutToBeDestroyed()), s, SLOT(aboutToBeDestroyed()) );
    if( ret )
    {
