...
HB_FUNC( QROTATIONSENSOR_ONHASZCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "hasZChanged(bool)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(hasZChanged(bool)), s, SLOT(hasZChanged(bool)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "hasZChanged(bool)";
    ret = object->disconnect(object, SIGNAL(hasZChanged(bool)), s, SLOT(hasZChanged(bool)) );
    if( ret )
    {
