...
HB_FUNC( QFONTDIALOG_ONCURRENTFONTCHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "currentFontChanged(QFont)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(currentFontChanged(QFont)), s, SLOT(currentFontChanged(QFont)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "currentFontChanged(QFont)";
    ret = object->disconnect(object, SIGNAL(currentFontChanged(QFont)), s, SLOT(currentFontChanged(QFont)) );
    if( ret )
    {
...
HB_FUNC( QFONTDIALOG_ONFONTSELECTED )
{
  ...
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "fontSelected(QFont)";
    bool connected = Signals_is_signal_connected( object, signal );
    if( !connected )
    {
      PHB_ITEM codeblock = hb_itemNew( hb_param( 1, HB_IT_BLOCK | HB_IT_SYMBOL ) );
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(fontSelected(QFont)), s, SLOT(fontSelected(QFont)) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "fontSelected(QFont)";
    ret = object->disconnect(object, SIGNAL(fontSelected(QFont)), s, SLOT(fontSelected(QFont)) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
