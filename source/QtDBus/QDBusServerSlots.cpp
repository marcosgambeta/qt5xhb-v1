...
HB_FUNC( QDBUSSERVER_ONNEWCONNECTION )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "newConnection(QDBusConnection)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(newConnection(QDBusConnection)), s, SLOT(newConnection(QDBusConnection)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "newConnection(QDBusConnection)";
    ret = object->disconnect(object, SIGNAL(newConnection(QDBusConnection)), s, SLOT(newConnection(QDBusConnection)) );
    if( ret )
    {
