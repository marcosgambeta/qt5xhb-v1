...
HB_FUNC( QTREEVIEW_ONCOLLAPSED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "collapsed(QModelIndex)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(collapsed(QModelIndex)), s, SLOT(collapsed(QModelIndex)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "collapsed(QModelIndex)";
    ret = object->disconnect(object, SIGNAL(collapsed(QModelIndex)), s, SLOT(collapsed(QModelIndex)) );
    if( ret )
    {
...
HB_FUNC( QTREEVIEW_ONEXPANDED )
{
  ...
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "expanded(QModelIndex)";
    bool connected = Signals_is_signal_connected( object, signal );
    if( !connected )
    {
      PHB_ITEM codeblock = hb_itemNew( hb_param( 1, HB_IT_BLOCK | HB_IT_SYMBOL ) );
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(expanded(QModelIndex)), s, SLOT(expanded(QModelIndex)) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "expanded(QModelIndex)";
    ret = object->disconnect(object, SIGNAL(expanded(QModelIndex)), s, SLOT(expanded(QModelIndex)) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
