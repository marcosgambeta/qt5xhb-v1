...
HB_FUNC( QSHORTCUT_ONACTIVATED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(activated()), s, SLOT(activated()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "activated()";
    ret = object->disconnect(object, SIGNAL(activated()), s, SLOT(activated()) );
    if( ret )
    {
...
HB_FUNC( QSHORTCUT_ONACTIVATEDAMBIGUOUSLY )
{
  ...
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "activatedAmbiguously()";
    bool connected = Signals_is_signal_connected( object, signal );
    if( !connected )
    {
      PHB_ITEM codeblock = hb_itemNew( hb_param( 1, HB_IT_BLOCK | HB_IT_SYMBOL ) );
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(activatedAmbiguously()), s, SLOT(activatedAmbiguously()) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "activatedAmbiguously()";
    ret = object->disconnect(object, SIGNAL(activatedAmbiguously()), s, SLOT(activatedAmbiguously()) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
