...
HB_FUNC( QABSTRACTEVENTDISPATCHER_ONABOUTTOBLOCK )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(aboutToBlock()), s, SLOT(aboutToBlock()) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "aboutToBlock()";
    ret = object->disconnect(object, SIGNAL(aboutToBlock()), s, SLOT(aboutToBlock()) );
    if( ret )
    {
...
HB_FUNC( QABSTRACTEVENTDISPATCHER_ONAWAKE )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(awake()), s, SLOT(awake()) );
        if( ret )
        {
          Signals_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          hb_retl(ret);
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "awake()";
    ret = object->disconnect(object, SIGNAL(awake()), s, SLOT(awake()) );
    if( ret )
    {
      Signals_disconnect_signal( object, signal );
      hb_retl(true);
