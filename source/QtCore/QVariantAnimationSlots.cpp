...
HB_FUNC( QVARIANTANIMATION_ONVALUECHANGED )
{
  ...
  if( hb_pcount() == 1 )
  {
    ...
    QString signal = "valueChanged(QVariant)";
    ...
    if( !connected )
    {
      ...
      if( codeblock )
      {
        ret = object->connect( object, SIGNAL(valueChanged(QVariant)), s, SLOT(valueChanged(QVariant)) );
        if( ret )
        {
  ...
  else if( hb_pcount() == 0 )
  {
    QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
    QString signal = "valueChanged(QVariant)";
    ret = object->disconnect(object, SIGNAL(valueChanged(QVariant)), s, SLOT(valueChanged(QVariant)) );
    if( ret )
    {
